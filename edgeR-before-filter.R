...
df_ret <- df_ret %>% select(tissue, trait, sequence, logFC, logCPM, PValue, FDR) 


df_ret %>% View
df_ret %>% write_csv("results/edgeR.csv")

p0 <- df_ret %>%
  ggplot(aes(x=trait, y=-log10(FDR), fill=trait)) +
...
p1 <- df_ret %>%
  ...
  facet_grid(tissue~sequence) + ggtitle("edgeR - FDR")
...
p2 <- df_ret %>% 
  ...
  facet_grid(tissue~sequence) + ggtitle("edgeR - effect size")
