...
DE_expansive_matrix <- dcast(viralLevel_DE, sequence ~ tissue_comparison_compound_id, value.var = "t", fill = 0)
...
dataValues <- sort(unique(c(unlist(DE_expansive_matrix))),decreasing=FALSE)
...
p1 <- pheatmap(gaps_col = colGaps, mat = DE_expansive_matrix[retainVir,orderCols], cellwidth = 45, cellheight = 20, 
         #filename =  "plots/MSBB_viral_level_RNA_DE_collectively_significant_FDR0.1.pdf",
         color=colSeq,breaks=bks,
         show_rownames = TRUE, display_numbers = pvalMat_label_df[retainVir,orderCols],border_color="white",scale="none", cluster_rows = TRUE, cluster_cols = FALSE,
...
p2 <- pheatmap(gaps_col = colGaps, mat = DE_expansive_matrix[retainVir,orderCols], cellwidth = 45, cellheight = 20, 
               #filename =  "plots/MSBB_viral_level_RNA_DE_collectively_significant_FDR0.1.pdf",
               color=colSeq,breaks=bks,
               show_rownames = TRUE, display_numbers = pvalMat_label_df[retainVir,orderCols],border_color="white",scale="none", 
